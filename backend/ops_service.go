...
func (s *OpsService) Search(ctx context.Context, q OpsQuery) (OpsPage, error) {
	...
	scratch := s.scratch
	if scratch == nil {
		scratch = make([]OpsRecord, 0, len(items))
	}
	filtered := opsFilterScratch(scratch[:0], items, q)
	s.scratch = filtered
	sortOpsRecords(filtered)
	q = opsQueryDefaults(q)
	start, end := opsBounds(len(filtered), q.Page, q.PageSize)
	page := OpsPage{Items: filtered[start:end], Page: q.Page, PageSize: q.PageSize, Total: len(filtered), HasNext: end < len(filtered)}
	return page, nil
}
...
func (s *OpsService) Snapshot() OpsSnapshot {
	...
	cached := append([]OpsRecord(nil), s.snapItems...)
	cached = append(cached, snapItems...)
	s.snapItems = cached
	for _, i := range s.snapItems {
		out.Records++
